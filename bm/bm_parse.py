...
def _ruamel_load(contents):
    try:
        loaded = ruamel.yaml.load(contents, Loader=ruamel.yaml.Loader)
    except AttributeError:
        loaded = ruamel.yaml.YAML(typ='rt').load(contents)
    return loaded
...
class RuamelYamlParse(BmCaseRun):
    ...
    def parse(self, case):
        _ = _ruamel_load(case.src_as_str)
...
class RuamelYamlEmit:
    ...
    def __init__(self, case):
        case.ruamel_emittree = _ruamel_load(case.src_as_str)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        raise Exception("missing argument: file name")
    filename = sys.argv[1]
    if filename.endswith("outer1000_inner1000.yml"):  # this one is too heavy for the Python libs
        exit(0)
    bmcase = BmCase(filename)
    run(bmcase, benchmarks=('parse', ),
    ...
    run(bmcase, benchmarks=('emit_yaml', 'emit_json', ),
